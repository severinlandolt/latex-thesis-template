...
Sp??hel.  It is reprinted here so that the template can serve as a
...
\begin{quote}
  ...
  \emph{Ein Graph ist nicht \(3\)-f??rbbar, wenn er eine \(4\)-Clique
    enth??lt.}
\end{quote}
...
\begin{quote}
  ...
  \emph{Wenn ein Graph eine \(4\)-Clique enth??lt, ist er nicht
    \(3\)-f??rbbar.}
\end{quote}
...
\begin{table}
  ...
  \begin{tabular}{lll}
    ...
    \st{$x$ fulfills property $\mathcal{P}$.}& \(x\) satisfies property \(\mathcal{P}\). & \emph{\(x\) erf??llt Eigenschaft \(\mathcal{P}\).} \\
    \st{in average} & on average & \emph{im Durchschnitt}\\
    \st{estimation} & estimate   & \emph{Absch??tzung}\\
